...
template <typename Type>
ConstMatrixIterator<Type>::ConstMatrixIterator(
const shared_ptr<typename Matrix<Type>::MatrixRow[]>& matrix_ptr, size_t rows,
size_t cols, size_t ind)
...
void ConstMatrixIterator<Type>::check_nullptr() const {
  if (matrix.expired()) {
    throw Access_Non_Existing_Exception(__FILE__, __LINE__, __TIME__,
...
template <typename Type>
bool ConstMatrixIterator<Type>::operator==(
ConstMatrixIterator const& other) const {
