...
Matrix<Type>::alloc_matrix(size_t rows, size_t cols) {
  ...
  try {
    ...
    //        throw Memory_Allocation_Exception(__FILE__, __LINE__, __TIME__,
...
template <typename Type>
Matrix<Type>::Matrix(
std::initializer_list<std::initializer_list<Type> > init_list) {
  ...
  for (const auto& row_list : init_list)
    if (row_list.size() != cols) {
      //            throw Wrong_Init_List_Exception(__FILE__, __LINE__,
      //            __TIME__, "wrong size of initializer row (must be equal)");
    }
...
Matrix<Type>::Matrix(const Matrix& matrix)
    : MatrixBase(matrix.rows, matrix.cols) {
...
template <typename Type>
const typename Matrix<Type>::MatrixRow&
...
Matrix<Type> Matrix<Type>::operator+(const Matrix<Type>& matrix) const {
  if (matrix.rows != rows || matrix.cols != cols) {
    throw Sizes_Differ_Exception(__FILE__, __LINE__, __TIME__,
                                 "impossible to make operation: sizes of "
...
template <typename Type>
Matrix<Type> Matrix<Type>::make_minor(const Matrix<Type>& matr,
                                      size_t crossed_row,
                                      size_t crossed_col) const {
...
template <typename Type>
void Matrix<Type>::resize(size_t new_rows, size_t new_cols,
                          const Type& filler) {
...
void Matrix<Type>::insert_row(size_t pos, const Type& filler) {
  if (pos > rows)
    throw Index_Out_Of_Range_Exception(__FILE__, __LINE__, __TIME__,
                                       "impossible to make operation: position "
                                       "is out of range");
...
ostream& operator<<(ostream& os, const Matrix<Type>& matr) {
  size_t i = 0;
  for (ConstMatrixIterator<Type> iter = matr.cbegin(); iter != matr.cend();
       iter++, i++) {
    *iter = 0;
